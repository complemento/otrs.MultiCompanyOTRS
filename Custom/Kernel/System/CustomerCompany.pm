...
sub CustomerCompanySourceList {
    ...
    for my $Count ( '', 1 .. 10 ) {
        ...
        my $Group = "UserIsGroup[" . $ConfigObject->Get("CustomerCompany$Count")->{Group} . "]";
        next SOURCE if !$ENV{MainSelf}->{$Group};
...
sub CustomerCompanyList {
    ...
    for my $Count ( '', 1 .. 10 ) {
        ...
        my $Group = "UserIsGroup[" . $Self->{"CustomerCompany$Count"}->{CustomerCompanyMap}->{Group} . "]";
        next SOURCE if !$ENV{MainSelf}->{$Group};


        #$Kernel::OM->Get('Kernel::System::Log')->Log( Priority => 'error', Message => "aaaaaaaaaaaa ".Dumper($Self->{"CustomerCompany$Count"}));
